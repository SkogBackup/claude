...
```bash
INIT=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE_ARG}")
...
```bash
AUDITOR_MODEL=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.nyquist_validation --raw)
```
...
```
Task(
  prompt="Read /home/skogix/claude/.claude/agents/gsd-nyquist-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `/home/skogix/claude/.claude/get-shit-done/templates/VALIDATION.md`
...
node "/home/skogix/claude/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
