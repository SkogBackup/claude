...
title: sandboxing
type: note
permalink: claude/docs/claude-code/sandboxing
---
...
1. **Reducing permission prompts**: Safe commands within the sandbox don't require approval
1. **Maintaining security**: Attempts to access resources outside the sandbox trigger immediate notifications
1. **Enabling autonomy**: Claude Code can run more independently within defined limits
...
- `docker` is incompatible with running in the sandbox. Consider specifying `docker *` in `excludedCommands` to force it to run outside of the sandbox. </Tip>
...
You can disable this escape hatch by setting `"allowUnsandboxedCommands": false` in your [sandbox settings](/en/settings#sandbox-settings). When disabled, the `dangerouslyDisableSandbox` parameter is completely ignored and all commands must run sandboxed or be explicitly listed in `excludedCommands`. </Note>
...
1. **Monitor logs**: Review sandbox violation attempts to understand Claude Code's needs
1. **Use environment-specific configs**: Different sandbox rules for development vs. production contexts
1. **Combine with permissions**: Use sandboxing alongside IAM policies for comprehensive security
1. **Test configurations**: Verify your sandbox settings don't block legitimate workflows
