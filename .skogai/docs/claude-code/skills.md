...
title: skills
type: note
permalink: claude/docs/claude-code/skills
---
...
**Custom commands have been merged into skills.** A file at `.claude/commands/deploy.md` and a skill at `.claude/skills/deploy/SKILL.md` both create `/deploy` and work the same way. Your existing `.claude/commands/` files keep working. Skills add optional features: a directory for supporting files, frontmatter to [control whether you or Claude invokes them](#control-who-invokes-a-skill), and the ability for Claude to load them automatically when relevant. </Note>
...
1. The output replaces the placeholder in the skill content
1. Claude receives the fully-rendered prompt with actual PR data
...
1. The subagent receives the skill content as its prompt ("Research $ARGUMENTS thoroughly...")
1. The `agent` field determines the execution environment (model, tools, and permissions)
1. Results are summarized and returned to your main conversation
...
1. Verify the skill appears in `What skills are available?`
1. Try rephrasing your request to match the description more closely
1. Invoke it directly with `/skill-name` if the skill is user-invocable
...
1. Add `disable-model-invocation: true` if you only want manual invocation
