...
title: model-config
type: note
permalink: claude/docs/claude-code/model-config
---
...
1. **At startup** - Launch with `claude --model <alias|name>`
1. **Environment variable** - Set `ANTHROPIC_MODEL=<alias|name>`
1. **Settings** - Configure permanently in your settings file using the `model` field.
...
1. In `/status`, which also displays your account information.
