...
type: note
permalink: claude/docs/claude-code/plugins-reference
---
...
1. Verify the shebang line: First line should be `#!/bin/bash` or `#!/usr/bin/env bash`
1. Check the path uses `${CLAUDE_PLUGIN_ROOT}`: `"command": "${CLAUDE_PLUGIN_ROOT}/scripts/your-script.sh"`
1. Test the script manually: `./scripts/your-script.sh`

**Hook not triggering on expected events**:

1. Verify the event name is correct (case-sensitive): `PostToolUse`, not `postToolUse`
1. Check the matcher pattern matches your tools: `"matcher": "Write|Edit"` for file operations
1. Confirm the hook type is valid: `command`, `http`, `prompt`, or `agent`
...
1. Verify all paths use `${CLAUDE_PLUGIN_ROOT}` variable
1. Check the MCP server logs: `claude --debug` shows initialization errors
1. Test the server manually outside of Claude Code
...
1. Check for connection timeouts in debug output
...
1. Check that each component directory is listed in the debug output
1. Verify file permissions allow reading the plugin files
...
If your plugin is within a [marketplace](/en/plugin-marketplaces) directory, you can manage the version through `marketplace.json` instead and omit the `version` field from `plugin.json`. </Warning>
