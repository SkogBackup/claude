...
title: hooks
type: note
permalink: claude/docs/claude-code/hooks
---
...
1. Add a [matcher group](#matcher-patterns) to filter when it fires, like "only for the Bash tool"
1. Define one or more [hook handlers](#hook-handler-fields) to run when matched
...
1. The hook returns `permissionDecision: "defer"`. The tool does not execute. The process exits with `stop_reason: "tool_deferred"` and the pending tool call preserved in the transcript.
1. The calling process reads `deferred_tool_use` from the SDK result, surfaces the question in its own UI, and waits for an answer.
1. The calling process runs `claude -p --resume <session-id>`. The same tool call fires `PreToolUse` again.
1. The hook returns `permissionDecision: "allow"` with the answer in `updatedInput`. The tool executes and Claude continues.
...
1. The LLM responds with structured JSON containing a decision
1. Claude Code processes the decision automatically
...
1. The subagent can use tools like Read, Grep, and Glob to investigate
1. After up to 50 turns, the subagent returns a structured `{ "ok": true/false }` decision
1. Claude Code processes the decision the same way as a prompt hook
