...
title: vs-code
type: note
permalink: claude/docs/claude-code/vs-code
---
...
1. **Check VS Code version**: Requires 1.98.0 or higher (Help → About)
1. **Restart VS Code**: Run "Developer: Reload Window" from the Command Palette
1. **Disable conflicting extensions**: Temporarily disable other AI extensions (Cline, Continue, etc.)
1. **Check workspace trust**: The extension doesn't work in Restricted Mode
...
1. **Start a new conversation**: Try starting a fresh conversation to see if the issue persists
1. **Try the CLI**: Run `claude` from the terminal to see if you get more detailed error messages
