...
type: note
permalink: claude/docs/claude-code/troubleshooting
---
...
1. **If behind a proxy**, set `HTTPS_PROXY` so the installer can route through it. See [proxy configuration](/en/network-config#proxy-configuration) for details.
...
1. **Use a larger instance** if possible. Claude Code requires at least 4 GB of RAM.
...
1. **Update macOS** if you're on an older version. The binary uses load commands that older macOS versions don't support.

1. **Try Homebrew** as an alternative install method:
...
If you find yourself repeatedly approving the same commands, you can allow specific tools to run without approval using the `/permissions` command. See [Permissions docs](/en/permissions#manage-permissions).
...
1. Close Claude Code
1. Restart with `claude` and complete the authentication process again
...
1. Close and restart Claude Code between major tasks
1. Consider adding large build directories to your `.gitignore` file
...
1. Run `/compact` with a focus that drops the large output, for example `/compact keep only the plan and the diff`
1. Move the large-file work to a [subagent](/en/sub-agents) so it runs in a separate context window
1. Run `/clear` if the earlier conversation is no longer needed
...
1. If unresponsive, you may need to close the terminal and restart
...
1. **Move project to Linux filesystem**: if possible, ensure your project is located on the Linux filesystem (`/home/`) rather than the Windows filesystem (`/mnt/c/`).

1. **Use native Windows instead**: consider running Claude Code natively on Windows instead of through WSL, for better file system performance.
...
1. **Use post-processing hooks**: set up automatic formatting hooks to detect and add missing language tags. See [Auto-format code after edits](/en/hooks-guide#auto-format-code-after-edits) for an example of a PostToolUse formatting hook.

1. **Manual verification**: after generating markdown files, review them for proper code block formatting and request corrections if needed.
...
1. **Use formatting tools**: set up hooks to run markdown formatters like `prettier` or custom formatting scripts on generated markdown files.

1. **Specify formatting preferences**: include formatting requirements in your prompts or project [memory](/en/memory) files.
...
1. Check the [GitHub repository](https://github.com/anthropics/claude-code) for known issues
1. Run `/doctor` to diagnose issues. It checks:
...
1. Ask Claude directly about its capabilities and features - Claude has built-in access to its documentation
