...
title: mcp
type: note
permalink: claude/docs/claude-code/mcp
---
...
This prevents conflicts between Claude's flags and the server's flags. </Note>
...
- Use `/mcp` to authenticate with remote servers that require OAuth 2.0 authentication </Tip>
...
Without the `cmd /c` wrapper, you'll encounter "Connection closed" errors because Windows cannot directly execute `npx`. (See the note above for an explanation of the `--` parameter.) </Warning>
...
1. Project scope
1. User scope
1. [Plugin-provided servers](/en/plugins)
1. [claude.ai connectors](#use-mcp-servers-from-claude-ai)
...
- OAuth authentication works with HTTP servers </Tip>
...
- Use `claude mcp get <name>` to verify that OAuth credentials are configured for a server </Tip>
...
- You can use `--scope user` to add the server to your user configuration instead of the project-specific one </Tip>
...
- If servers with the same names already exist, they will get a numerical suffix (for example, `server_1`) </Tip>
...
Without the correct executable path, you'll encounter errors like `spawn claude ENOENT`. </Warning>
...
- Note that this MCP server is only exposing Claude Code's tools to your MCP client, so your own client is responsible for implementing user confirmation for individual tool calls. </Tip>
...
- Resources can contain any type of content that the MCP server provides (text, JSON, structured data, etc.) </Tip>
...
- Server and prompt names are normalized (spaces become underscores) </Tip>
...
1. **Policy-based control with allowlists/denylists**: Allow users to add their own servers, but restrict which ones are permitted
...
1. **By command** (`serverCommand`): Matches the exact command and arguments used to start stdio servers
1. **By URL pattern** (`serverUrl`): Matches remote server URLs with wildcard support
...
- Stdio server with any command: ❌ Blocked (no name or command entries to match) </Accordion>
...
- HTTP server named "my-api": ❌ Blocked (no name entries to match) </Accordion>
...
- HTTP server named "other-api": ❌ Blocked (name doesn't match) </Accordion>
...
- HTTP server named "github": ✅ Allowed (matches name)
- Any server named "other": ❌ Blocked (name doesn't match) </Accordion>
