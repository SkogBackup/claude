...
title: chrome
type: note
permalink: claude/docs/claude-code/chrome
---
...
1. Verify Claude Code is up to date by running `claude --version`
1. Check that Chrome is running
1. Run `/chrome` and select "Reconnect extension" to re-establish the connection
1. If the issue persists, restart both Claude Code and Chrome
...
1. Ask Claude to create a new tab and try again
1. Restart the Chrome extension by disabling and re-enabling it in `chrome://extensions`
