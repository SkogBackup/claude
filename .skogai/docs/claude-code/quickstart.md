...
title: quickstart
type: note
permalink: claude/docs/claude-code/quickstart
---
...
@media (max-width: 720px) { .cc-ic-tab { padding: 12px 14px; font-size: 14px; } .cc-ic-sales-actions { width: 100%; } .cc-ic-card-body { padding: 20px; } .cc-ic-cmd { font-size: 15px; } } \`; return <div className="cc-ic not-prose"> <style>{STYLES}</style>
...
}, { k: 'cmd', label: 'CMD' }\].map(({k, label}) => { const active = k === 'cmd' === winCmd; return \<button key={k} type="button" role="tab" aria-selected={active} className={'cc-ic-shell-option' + (active ? ' cc-ic-active' : '')} onClick={() => setWinCmd(k === 'cmd')}> {label} </button>; })} </div>} {cardBodyCmd(terminalCmd, isWinPrompt ? '>' : '$')} </div>}
