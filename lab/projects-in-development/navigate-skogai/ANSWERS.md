...
title: ANSWERS
type: note
permalink: claude/lab/projects-in-development/navigate-skogai/answers
---
...
1. **setgid bit**: the `s` in `drwxrwsr-x` means any file created in `/skogai` automatically inherits the `skogai` group
1. **group members**: both humans (skogix, aldervall) and AI agents as system users (dot, amy, claude, goose)
...
**the design intent** SkogAI is fundamentally a multi-agent system where AI agents operate as actual system users with home directories. `/skogai` is their shared collaboration space where permissions "just work" through Unix group mechanics rather than complex access control.
...
1. examining special bits (`file /skogai`)
1. inspecting group membership (`getent group skogai`)
1. reasoning about what these technical facts imply
