...
**Session ID**: session_011iwfyHqnjkTAUdvYTjPrp2
**Container ID**: container_019vvRENrjWDJ3Q7Uo66yxPs--claude_code_remote--shy-mixed-juicy-radian
...
2. **Create cloud-specific config** paths that respect `$CLAUDE_CODE_REMOTE`
3. **Document both modes** in README with usage examples
4. **Use relative paths** where possible instead of hardcoded `/skogai`
5. **Test in both environments** to ensure compatibility

### For Context System
1. **Enhance tmp/context** to include environment metadata
2. **Auto-detect mode** and adjust git-diff communication accordingly
3. **Store session info** to understand which environment generated context
4. **Version control cloud vs local** context separately if needed

### For Documentation
1. Create `docs/environments/cloud.md` (this environment)
2. Create `docs/environments/local.md` (intended setup)
3. Create `docs/environments/detection.md` (how to detect and adapt)
4. Update CLAUDE.md with environment-aware instructions
...
2. Repository cloned/synced to `/home/user/skogai`
3. Environment variables set (session-specific tokens, IDs)
4. Claude Code agent starts with remote entrypoint
5. User connects via mobile app or web interface
