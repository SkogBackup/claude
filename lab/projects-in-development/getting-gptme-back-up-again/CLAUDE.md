...
title: CLAUDE
type: note
permalink: claude/lab/projects-in-development/getting-gptme-back-up-again/claude
---

# getting-gptme-back-up-again

\<what_is_this> Project to setup gptme and related tooling in a way that makes sense, with clear understanding of why things work the way they do. \</what_is_this>
...
1. Tools execute and feed results back
1. Loop continues until task complete
...
1. **Tool-Driven Architecture** - AI capabilities = composable tools
1. **Local-First** - Prefer local execution and models
1. **Unconstrained** - Full control over behavior (vs. commercial alternatives)
1. **Clean & Typed** - Type hints, mypy checked, ruff formatted
...
1. **Extend safely** - Add capabilities without breaking core
1. **Configure properly** - Understand layered configuration
1. **Optimize performance** - Know what's expensive (LLM calls, context size)
1. **Choose the right approach** - Tools vs hooks vs plugins vs commands
...
1. **Task Selection** (5-10 min) - Use CASCADE to pick next task
1. **Execution** (20-30 min) - Do the work
...
1. Clears personal content (old tasks, journal, knowledge)
1. Updates agent name in gptme.toml
1. Creates ABOUT.md template
1. Initializes fresh identity
1. Maintains all tooling and scripts
...
1. **Maintain context** - Structured knowledge prevents information loss
1. **Enable autonomy** - Safe, scheduled operation without human oversight
1. **Scale agent teams** - Fork template to create specialized agents
1. **Track evolution** - Git history shows agent's learning and decisions
