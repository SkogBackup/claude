...
2. **Gather Context** — Extract from conversation: module name, symptom, failed attempts, root cause, solution, prevention
3. **Check Existing Docs** — Search for similar issues; offer user choice to create new or update existing
4. **Generate Filename** — Sanitized format: `[symptom]-[module]-[YYYYMMDD].md`
5. **Validate YAML Schema** — BLOCKING gate enforces enum validation (problem_type, component, root_cause, resolution_type, severity)
6. **Create Documentation** — Populate using template from `assets/resolution-template.md` with validated YAML
7. **Cross-Reference & Detect Patterns** — Link related issues; suggest critical pattern promotion if 3+ similar issues exist
...
**Decision Gate After Documentation:**
Present 7 options post-completion:
1. Continue workflow (default)
2. Add to Required Reading (promote to critical patterns)
3. Link related issues (cross-reference)
4. Add to existing skill (attach to learning skill)
5. Create new skill (extract as new domain)
6. View documentation (display created doc)
7. Other (user-defined)
